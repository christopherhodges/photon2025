import fetch from 'node-fetch';
import Papa from 'papaparse';

const SHEET_CSV =
  'https://docs.google.com/spreadsheets/d/1PEBrV4PcfjV6IIIrkpjUrrh1ipXPds7_WBI6EBDl24c/export?format=csv';

// Map one row → one Next.js redirect object
function rowToRedirect({ Old, New, Type }) {
  if (!Old || !New) return null;

  const { hostname, pathname, search } = new URL(Old);

  // Only attach the rule if it belongs to the production host
  // (adjust regex for www/non-www as needed)
  const hostRegex = '^(?:www\\.)?photon\\.health$';
  if (!new RegExp(hostRegex).test(hostname)) return null;

  return {
    source: `${pathname.replace(/\/$/, '')}${search || ''}` || '/',
    destination: New,
    permanent: String(Type || '').trim() !== '302',
    has: [
      {
        type: 'host',
        value: hostRegex, // <-- host guard
      },
    ],
  };
}

export async function fetchSheetRedirects() {
  const csv = await (await fetch(SHEET_CSV)).text();
  const { data } = Papa.parse(csv, { header: true, skipEmptyLines: true });
  return data.map(rowToRedirect);
}
